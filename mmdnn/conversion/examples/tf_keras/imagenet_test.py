...
class TestTFKeras(TestKit):
    ...
    def __init__(self):
        ...
        super(TestTFKeras, self).__init__()
        self.model = self.MainModel.KitModel(self.args.w)


    def preprocess(self, image_path):
        x = super(TestTFKeras, self).preprocess(image_path)
        self.data = np.expand_dims(x, 0)

    def print_result(self):
        predict = self.model.predict(self.data)
        super(TestTFKeras, self).print_result(predict)
    ...
    def print_intermediate_result(self, layer_name, if_transpose = False):
        ...
        super(TestTFKeras, self).print_intermediate_result(intermediate_output, if_transpose)
